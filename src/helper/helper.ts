import { StateTokenResponse } from "@/redux/authSlice";
import { makeStyles } from "@rneui/themed";
import { TokenResponse } from "expo-auth-session";

export function sanitizeName(name: string) {
  if (!name) return;
  name = Array.from(new Set(name.toLowerCase().split(" ") || [])).join(" ");
...
export const useAppStyles = makeStyles((theme, props: StyleProps) => ({
  container: {
    flex: 1,
...
export function convertTokenResponseToStateTokenResponse(tokenResponse: TokenResponse): StateTokenResponse {
